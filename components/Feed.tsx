...
import { MdTurnedInNot, MdDelete } from "react-icons/md";
...
const Feed: React.FC<FeedProps> = ({ data }) => {
  const [alreadySaved, setAlreadySaved] = useState<boolean | null>(null);
  ...
  const saveToCollections = async (id: string, uid: string | undefined) => {
    if (!alreadySaved) {
